...
class Image(UserDict):
    ...
    def create(data, imageRequest=None, filename=None):
        ...
        if imageRequest:
            try:
                ...
                if not filename:
                    raise TypeError(
                        "Invalid filename, you must pass filename=something" "or a valid ImageRequest object"
                    )
    ...
    def extract(self, params={}, saveCatalog=False, saveConfig=False):
        ...
        sex.config["PARAMETERS_LIST"] = ["NUMBER", "XWIN_IMAGE", "YWIN_IMAGE", "FLUX_BEST", "FWHM_IMAGE", "FLAGS"]
