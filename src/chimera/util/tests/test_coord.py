...
class TestCoord(object):
    ...
    def test_parsing_conversion_bsc(self):
        ...
        bsc = ascii.read(
            os.path.abspath(os.path.join(os.path.dirname(__file__), "bsc.dat")), format="tab", converters={}
        )
        ...
        for i in range(len(bsc)):
            ...
            assert TestCoord.equal(ra[i].D, expected_ra[i], e=1e-4), "ra: %.6f != coord ra: %.6f (%.6f)" % (
                expected_ra[i],
                ra[i].D,
                expected_ra[i] - ra[i].D,
            )

            assert TestCoord.equal(dec[i].D, expected_dec[i], e=1e-4), "dec: %.6f != coord dec: %.64f (%.6f)" % (
                expected_dec[i],
                dec[i].D,
            ...
            assert expected_ra_str[i] == ra[i].strfcoord("%(h)02d %(m)02d %(s)04.1f"), "ra: %s != coord ra: %s" % (
                expected_ra_str[i],
                ra[i].strfcoord("%(h)02d %(m)02d %(s)04.1f"),
            )

            assert expected_dec_str[i] == dec[i].strfcoord("%(d)02d %(m)02d %(s)02.0f"), "dec: %s != coord dec: %s" % (
                expected_dec_str[i],
    ...
    def test_parsing_conversion_hipparcos(self):
        ...
        hipp = ascii.read(
            os.path.abspath(os.path.join(os.path.dirname(__file__), "hipparcos-tycho.dat")), format="tab"
        )
        ...
        for i in range(len(hipp)):
            assert expected_ra_str[i] == ra_hms[i].strfcoord("%(h)02d %(m)02d %(s)05.2f"), "ra: %s != coord ra: %s" % (
                expected_ra_str[i],
            ...
            assert TestCoord.equal(ra[i].D, expected_ra[i], e=1e-8), "ra: %.6f != coord ra: %.6f (%.6f)" % (
                expected_ra[i],
                ra[i].D,
                expected_ra[i] - ra[i].D,
            )

            assert TestCoord.equal(dec[i].D, expected_dec[i], e=1e-8), "dec: %.6f != coord dec: %.64f (%.6f)" % (
                expected_dec[i],
                dec[i].D,
                expected_dec[i] - dec[i].D,
            )
            ...
            assert TestCoord.equal(ra_hms[i].D, expected_ra[i], e=1e-4), "ra: %.6f != coord ra: %.6f (%.6f)" % (
                expected_ra[i],
                ra_hms[i].D,
                expected_ra[i] - ra_hms[i].D,
            )

            assert TestCoord.equal(dec_dms[i].D, expected_dec[i], e=1e-4), "dec: %.6f != coord dec: %.64f (%.6f)" % (
                expected_dec[i],
                dec_dms[i].D,
    ...
    def test_parse_dms(self):
        ...
        print(
            ...
            % (len(coords), t_parse, len(coords) / t_parse, t_check, len(coords) / t_check)
        )
