...
class CoordUtil(object):
    ...
    def coordRotate(x, y, z):
        ...
        """
        # -- 1 --
        xt = math.asin(math.sin(x) * math.sin(y) + math.cos(x) * math.cos(y) * math.cos(z))
        # -- 2 --
        yt = math.acos((math.sin(x) - math.sin(y) * math.sin(xt)) / (math.cos(y) * math.cos(xt)))
        # -- 3 --
        if math.sin(z) > 0.0:
            yt = TWO_PI - yt
...
class Coord(object):
    ...
    def __repr__(self):
        return "<%s object %s (%s) at %s>" % (Coord.__name__, str(self), self.state, hex(id(self))[:-1])
