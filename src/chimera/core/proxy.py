...
class ProxyMethod(object):
    ...
    def __repr__(self):
        return "<%s.%s method proxy at %s>" % (self.proxy.location, self.method, hex(hash(self)))
    ...
    def __do(self, other, action):
        ...
        try:
            ...
            log.exception("Cannot %s to topic '%s' using proxy '%s'." % (action, self.method, self.proxy))
