...
class ChimeraCLI(object):
    ...
    def __init__(
        self, prog, description, version, port=None, verbosity=True, instrument_path=True, controllers_path=True
    ):
    ...
    def _run(self, cmdlineArgs):
        ...
        self.options, args = self.parser.parse_args(
            cmdlineArgs, values=CLIValues(defaults=self.parser.get_default_values().__dict__)
        )
    ...
    def _startSystem(self, options):
        ...
        try:
            ...
            self._remoteManager = Manager.locate(self.sysconfig.chimera["host"], self.sysconfig.chimera["port"])
    ...
    def _createParser(self):
        ...
        for param in list(self._parameters.values()):
            ...
            option_kwargs = dict(action=option_action, dest=param.name, help=param.help, metavar=param.metavar)
    ...
    def _getActions(self, options):
        ...
        actions.extend([action for action in list(self._actions.values()) if action.default == True])
    ...
    def _validateParameters(self, options):
        ...
        paramValues = [getattr(options, param) for param in list(self._parameters.keys())]
