    ...
    def process(action):
        ...
        try:
            ...
            if action.target_ra_dec is not None:
                ...
                # Coord.deg), unlike .ra/.dec which are Coords - so the
                # .to_d() this used to call blew up on every alt/az point
                # action: "'float' object has no attribute 'to_d'". robobs'
                # SAFETY park is alt/az, so it failed every time the queue
                # emptied. hasattr keeps it working if a Coord shows up.
