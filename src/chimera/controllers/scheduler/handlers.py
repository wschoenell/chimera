...
class PointHandler(ActionHandler):
    ...
    def log(action):
        ...
        offsetNS_str = (
            ...
            else " north %s" % action.offsetNS if action.offsetNS > 0 else " south %s" % abs(action.offsetNS)
        )
        offsetEW_str = (
            ""
            if action.offsetEW is None
            else " west %s" % abs(action.offsetEW) if action.offsetEW > 0 else " east %s" % abs(action.offsetEW)
        )
