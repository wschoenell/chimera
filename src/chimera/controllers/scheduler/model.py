...
class Targets(Base):
    ...
    def __str__(self):
        if self.observed:
            return "#%d %s [type: %s] #LastObverved@: %s" % (self.id, self.name, self.type, self.lastObservation)
        else:
...
class Program(Base):
    ...
    actions = relation(
        "Action", backref=backref("program", order_by="Action.id"), cascade="all, delete, delete-orphan"
    )

    def __str__(self):
        return "#%d %s pi:%s #actions: %d" % (self.id, self.name, self.pi, len(self.actions))
...
class Point(Action):
    ...
    def __str__(self):
        offsetNS_str = (
            ...
            else " north %s" % self.offsetNS if self.offsetNS > 0 else " south %s" % self.offsetNS
        )
        offsetEW_str = (
            ""
            if self.offsetEW is None
            else " west %s" % self.offsetEW if self.offsetEW > 0 else " east %s" % self.offsetNS
        )

        offset = (
            "" if self.offsetNS is None and self.offsetEW is None else "offset: %s%s" % (offsetNS_str, offsetEW_str)
        )
