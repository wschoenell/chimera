...
class ThreadingXMLRPCServer(socketserver.ThreadingTCPServer, xmlrpc.server.SimpleXMLRPCDispatcher):

    def __init__(self, addr, requestHandler=xmlrpc.server.SimpleXMLRPCRequestHandler, logRequests=1, allow_none=True):

        self.logRequests = logRequests

        if sys.version_info[:2] == (2, 5):
            xmlrpc.server.SimpleXMLRPCDispatcher.__init__(self, False, sys.getdefaultencoding())
        else:
