...
class FocusFit(object):
    ...
    best_focus = property(lambda self: (-self.B / (2 * self.A), (-self.B**2 + 4 * self.A * self.C) / (4 * self.A)))
...
class Autofocus(ChimeraObject, IAutofocus):
    ...
    def focus(
        ...
        try:
            ...
            return {"current_run": self.currentRun, "A": fit.A, "B": fit.B, "C": fit.C, "best": int(fit.best_focus[0])}
