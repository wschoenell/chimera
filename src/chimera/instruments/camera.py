...
from chimera.interfaces.camera import CameraExpose, CameraTemperature, CameraInformation, InvalidReadoutMode, Shutter
...
class CameraBase(ChimeraObject, CameraExpose, CameraTemperature, CameraInformation):
    ...
    def _getReadoutModeInfo(self, binning, window):
        ...
        if not binning:
            binning = list(self.getBinnings().keys()).pop(list(self.getBinnings().keys()).index("1x1"))
    ...
    def getMetadata(self, request):
        ...
        if "frame_temperature" in list(self.extra_header_info.keys()):
            md += [
                ("CCD-TEMP", self.extra_header_info["frame_temperature"], "CCD Temperature at Exposure Start [deg. C]")
            ]
        ...
        if "frame_start_time" in list(self.extra_header_info.keys()):
            md += [
                (
                    "DATE-OBS",
                    ImageUtil.formatDate(self.extra_header_info.get("frame_start_time")),
        ...
        if focal_length is not None:  # If there is no telescope_focal_length defined, don't store WCS
            ...
            md += [
                ...
                ("CD1_1", scale_x * cos(self["rotation"] * pi / 180.0), "transformation matrix element (1,1)"),
                ("CD1_2", -scale_y * sin(self["rotation"] * pi / 180.0), "transformation matrix element (1,2)"),
                ("CD2_1", scale_x * sin(self["rotation"] * pi / 180.0), "transformation matrix element (2,1)"),
                ("CD2_2", scale_y * cos(self["rotation"] * pi / 180.0), "transformation matrix element (2,2)"),
            ]
