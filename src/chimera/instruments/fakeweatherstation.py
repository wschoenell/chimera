...
class FakeWeatherStation(
    WeatherBase, WeatherTemperature, WeatherHumidity, WeatherPressure, WeatherWind, WeatherRain, WeatherTransparency
):
    def __init__(self):
    ...
    def temperature(self, unit_out=units.Celsius):
        ...
        temperature = 25 * math.sin(self._hourinradians(current_time.hour) - math.pi / 2.0) + 15.0

        temperature = self._convert_units(
            temperature, units.Celsius, unit_out, equivalencies=units.equivalencies.temperature()
        )
    ...
    def wind_speed(self, unit_out=units.meter / units.second):
        ...
        speed = self._convert_units(reference_speed, units.meter / units.second, unit_out)
    ...
    def dew_point(self, unit_out=units.Celsius):
        ...
        temperature = self._convert_units(10, units.Celsius, unit_out, equivalencies=units.equivalencies.temperature())
